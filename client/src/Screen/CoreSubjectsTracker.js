...
const CoreSubjectsTracker = () => {
  ...
  useEffect(() => {
    ...
    const getAllQuestions=async(req,res)=>{
      try {
        const res=await axios.get('http://localhost:8000/coreSub/getQuestion',{
          headers:{
            'Authorization':token
          }
        })
      ...
      try {
        const updatedFilterData = topicsData.map((item) => {
          const completed = localStorage.getItem(
            `completedtopic-coresheet-${item._id}`
          );
        ...
        const updatedData = data.map((item) => {
          const completed = localStorage.getItem(
            `completedquestion-coresheet-${item._id}`
          );
          const marked = localStorage.getItem(
            `markedquestion-coresheet-${item._id}`
          );
        ...
        const filteredData = updatedData.filter((item) => {
          return (
            showAllQuestions ||
            (showSolvedQuestions && item.completed) ||
            (showMarkedQuestions && item.marked)
          );
        });
  ...
  const toggleCompleted = async (index) => {
    ...
    try {
      ...
      if (updatedData[index].completed ) {
        res=await axios.post(`http://localhost:8000/coreSub/addQuestion/${id}`,{
          id:id,
          email,
         ...
         res = await axios.patch(
          `http://localhost:8000/coreSub/addCoreSubject/${id}`,
          {
            id: id,
            email,
            completed: updatedData[index].completed,
  ...
  const completedTopicsCount = topicsData.filter(
    (topic) => topic.completed
  ).length;
  const totalTopicsCount = topicsData.length;

  const topicsProgressBarPercent =
    totalTopicsCount === 0
      ? 0
      : ((completedTopicsCount / totalTopicsCount) * 100).toFixed(0);
  const questionsProgressBarPercent =
    data.length === 0
      ? 0
      : (
          (filteredData.filter((item) => item.completed === true).length /
            data.length) *
          100
        ).toFixed(data.length > 100 ? 1 : 0);
  ...
  return (
    <GrandContainer needDarkMode={needDarkMode}>
      ...
      <Container needDarkMode={needDarkMode}>
        ...
        {needDarkMode ? (
          <LeftMenuDark marked={"core-subjects-tracker"} />
        ) : (
          <LeftMenu marked={"core-subjects-tracker"} />
        )}

        <div className="cc-middle-content">
          <h1 className="main-heading">Core Subjects Tracker</h1>
          <p className="heading-supporter">
            We've compiled a comprehensive set of interview questions sourced
            from reputable websites such as GeeksforGeeks and InterviewBit.
            Additionally, we've incorporated core subject knowledge shared by
            renowned YouTubers like Striver, Fraz, etc. The questions undergo
            thorough parsing using AI to filter out the most relevant ones, and
            our AI system provides ideal candidate answers.
          </p>

          <Filters needDarkMode={needDarkMode}>
            <a
              href="https://github.com/Mohit030802/Algolisted-Group-119/tree/main/client/src/DummyDB/CoreSubjects"
              target="_blank"
              className="filter2"
            >
          ...
          <SheetMessage needDarkMode={needDarkMode}>
            <div className="text">
              Discover a goldmine of operating system resources on our website!
              As an open-source platform, we've handpicked the best content,
              from YouTube playlists to courses and websites, all designed to
              supercharge your preparation for placements. Your one-stop shop
              for all things {subjectName} – exclusively on our site!
            </div>
            <div
              className="open-btn"
            ...
            >
              {openVisualiser ? (
                <>
                  <div className="desc">Close Resources</div>
                  <ExpandLessIcon />
                </>
              ) : (
                <>
                  <div className="desc">Open Resources</div>
                  <ExpandMoreIcon />
                </>
              )}
            </div>
          ...
          <div className="topics-container">
            {topicsData.map((topic, index) => (
              <div className="topic" key={topic.name}>
                ...
                />
                <label htmlFor={topic.name.toLowerCase().replace(/\s+/g, "-")}>
                  {topic.name}
                </label>
              </div>
            ))}
          ...
          <EffectiveFilter needDarkMode={needDarkMode}>
            ...
            <div className="right">
              <a
                href="https://github.com/Nayaker/AlgoListed/blob/main/client/src/DummyDB/CoreSubjects/OSquestions.json"
                target="_blank"
                className="filter-item"
              >
                Contribute - New or Enhancement <CallMadeIcon />
              </a>
            </div>
          </EffectiveFilter>
                    ...
                    <div className="link-row-left">
                      ...
                      <div className="main-row-content">
                        ...
                        <div
                          ...
                          style={{
                            display: answerVisibility[index] ? "block" : "none",
                          }}
                        ></div>
                    ...
                    <div className="right-icons">
                      ...
                      >
                        <div className="review-btn">
                          <BookmarkIcon onClick={() => toggleMarked(index)} />
                        </div>
                      </Tooltip>
