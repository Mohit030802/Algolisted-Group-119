...
const CodingSheets = () => {
  ...
  const [sortedSolvedTopicTagsValues, setSortedSolvedTopicTagsValues] =
    useState([]);
  ...
  const progressBarPercent =
    data.length === 0
      ? 0
      : ((completedCount / data.length) * 100).toFixed(
          data.length > 100 ? 1 : 0
        );
  ...
  const toggleCompleted =async (id) => {
    ...
    var solvedQuestions = [];
    for (let i = 0; i < len; i++) {
      if (updatedData[i].completed == true)
        solvedQuestions.push(updatedData[i]);
    }
    ...
    try {
      ...
      console.log(updatedData[index].completed)
      if (updatedData[index].completed ) {
        res= await axios.post(`http://localhost:8000/coding/addCodingQuestion/${id}`,{
          id:id,
          email,
         ...
         res = await axios.patch(
          `http://localhost:8000/coding/updateCodingQuestion/${id}`,
          {
            id: id,
            email,
            completed: updatedData[index].completed,
  ...
  const handleReset = (index) => {
    ...
    localStorage.removeItem(
      `elapsedTimeSheetQuestion-${updatedData[index]._id}`
    );
  };
  ...
  const Stopwatch = ({ id, initialElapsed, isRunning }) => {
    ...
    useEffect(() => {
      if (isRunning && elapsedTime != 0) {
        localStorage.setItem(
          `elapsedTimeSheetQuestion-${updatedData[id]._id}`,
          elapsedTime
        );
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `https://algolisted.cyclic.app/coding-questions/question/${sheetname}`
      )
      .then((res) => {
        // retrieve the "completed" status of each sheet from the local storage
        let updatedData = res.data.map((sheet) => {
          const completed = localStorage.getItem(
            `completedSheetQuestion-${sheet._id}`
          );
          const marked = localStorage.getItem(
            `markedSheetQuestion-${sheet._id}`
          );
          const elapsedTime =
            localStorage.getItem(`elapsedTimeSheetQuestion-${sheet._id}`) || 0;
          return {
            ...sheet,
  ...
  useEffect(() => {
    ...
    const filteredTags = ProblemsTags.filter((tag) =>
      allowedProblemTags.includes(tag)
    );
  ...
  useEffect(() => {
    const newFilteredData = data.filter((item) => {
      ...
      if (
        selectedValue !== "All" &&
        selectedValue === "Marked for later" &&
        !item.marked
      ) {
        return false;
      }
      if (
        selectedValue !== "All" &&
        selectedValue === "Marked as completed" &&
        !item.completed
      ) {
        return false;
      }
  ...
  return (
    <GrandContainer>
      <MobContainer>
        ...
        <div className="main-content">
          ...
          <p className="heading-supporter">
            ...
            feature on the Algolisted website. Not only can you find a wide
            range of sheets all in one place, but the included analysis graphs
            make solving them even more enjoyable by allowing you to track your
            progress. Plus, a discussion section is coming soon to provide
            support and guidance as you work through each sheet. Happy coding!
          </p>
      ...
      <Container needDarkMode={needDarkMode}>
        ...
        {needDarkMode ? (
          <LeftMenuDark marked={"coding-sheets"} />
        ) : (
        ...
        <div className="cc-middle-content">
          ...
          <div className="message">
            ...
            <div className="text">
              As we continue to develop our platform, we do not currently
              require users to create accounts. As a result, any progress made
              is saved locally in your device's storage. Therefore, it's
              recommended to not clear your browser's cache.
            </div>
          </div>
          ...
          <SheetMessage needDarkMode={needDarkMode}>
            <div className="text">
              Hey there! With this tool, you can easily see a visual
              representation of the coding sheet you are working on and track
              your progress as you go. It also gives you an idea of the types of
              questions you can expect to find on the sheet. Cool, huh?
            </div>
            <div
              className="open-btn"
            ...
            >
              {openVisualiser ? (
                <>
                  <div className="desc">Close Visualiser</div>
                  <ExpandLessIcon />
                </>
              ) : (
                <>
                ...
                <div className="visualiser-conatiner">
                  <div className="canvas-container">
                    <div className="top-label">
                      <div
                        className={
                          showSolvedChart ? "label-item" : "label-item selected"
                        }
                        onClick={() => setShowSolvedChart(false)}
                      >
                        Problem Tags in Sheet
                      </div>
                      <div
                        className={
                          !showSolvedChart
                        ...
                        onClick={
                          solvedData.length > 0
                            ? () => setShowSolvedChart(true)
                            : () =>
                                alert(
                                  "You need to solve atleast one problem to use this feature."
                                )
                        }
                      >
                        Solved Tags
                            ...
                            return (
                              <div className="label" key={index}>
                                ...
                                <div className="label-key">
                                  {sortedSolvedTopicTagsKeys[index]} :{" "}
                                </div>
                                <div className="label-value">
                                  {sortedSolvedTopicTagsValues[index]}
                                </div>
                              </div>
                            );
                          })}
                    ...
                    <div className="category">
                      <div className="cat-data">
                        ...
                        <div className="completed">
                          ...
                          <div className="value">
                            {userDifficulty.Easy} / {difficulty.Easy}
                          </div>
                        </div>
                      </div>
                      <div className="line">
                    ...
                    <div className="category">
                      <div className="cat-data">
                        ...
                        <div className="completed">
                          <div className="text">Completed : </div>
                          <div className="value">
                            {userDifficulty.Medium} / {difficulty.Medium}
                          </div>
                        </div>
                      </div>
                      <div className="line">
                    ...
                    <div className="category">
                      <div className="cat-data">
                        ...
                        <div className="completed">
                          <div className="text">Completed : </div>
                          <div className="value">
                            {userDifficulty.Hard} / {difficulty.Hard}
                          </div>
                        </div>
                      </div>
                      <div className="line">
          ...
          <EffectiveFilter needDarkMode={needDarkMode}>
            <div className="left">
              ...
              <select
                className="filter-item"
                value={selectedValue}
                onChange={(e) => handleValueClick(e.target.value)}
              >
            ...
            <div className="right">
              ...
              >
                {showTags ? "Hide Problem Tags" : "Show Problem Tags"}
              </div>
          ...
          {filteredData.length != data.length ? (
            <i>
              <div className="notice">
                Filters are presently in the process of being developed, so they
                may occasionally exhibit unusual behavior.
              </div>
            </i>
          ) : (
            <div></div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="right-icons">
                      ...
                      <Tooltip
                        title={item.marked ? "Unmark" : "Mark for Later"}
                      >
                        <div className="review-btn">
                          <UpdateIcon onClick={() => toggleMarked(index)} />
                        </div>
                      </Tooltip>
  ...
  .cc-middle-content {
    ...
    .table {
          ...
          .main-row-content {
            .question-main {
              ...
              a {
                ...
                /* color: inherit; */
                color: ${(props) =>
                  props.needDarkMode ? "cornflowerblue" : "#18489f"};
            ...
            .tags {
              ...
              .tag {
                ...
                color: ${(props) => (props.needDarkMode ? "#e5e5e5" : "#333")};
                background-color: ${(props) =>
                  props.needDarkMode ? "#2b2b2b" : "#f3f4f7"};
              }
              ...
              .special-tag {
                ...
                font-weight: 500;
                border: 1px solid
                  ${(props) => (props.needDarkMode ? "#fff" : "#111")};
                color: ${(props) => (props.needDarkMode ? "#e5e5e5" : "#333")};
                /* background-color: ${(props) =>
                  props.needDarkMode ? "#2b2b2b" : "#f3f4f7"}; */
              }
      ...
      .review-row {
        ...
        position: relative;
        background-color: ${(props) =>
          props.needDarkMode ? "#2b2023" : "#ffe3e2"};
        border-radius: 0;
...
const Filters = styled.div`
  ...
  .filter {
    ...
    text-decoration: none;
    background-color: ${(props) =>
      props.needDarkMode ? "transparent" : "transparent"};
    color: ${(props) => (props.needDarkMode ? "#e5e5e5" : "#333")};
    ...
    &:hover {
      background-color: ${(props) =>
        props.needDarkMode ? "#4a4d5a" : "#f1f1f1"};
...
const VisualiserConatiner = styled.div`
  ...
  .visualiser-conatiner {
    ...
    .canvas-container {
      border: 1px solid
        ${(props) => (props.needDarkMode ? "#595b5f" : "#d1d5db")};
      background-color: ${(props) =>
        props.needDarkMode ? "#404249" : "rgba(255, 255, 255, 0.83)"};
    ...
    .learn-self-graph {
      ...
      .category {
        ...
        .line {
          ...
          overflow: hidden;
          border: 1px solid
            ${(props) => (props.needDarkMode ? "#000000" : "#f1f6f1")};
          background-color: ${(props) =>
            props.needDarkMode ? "#2b2d31" : "#dbd5d5"};
          /* background-color: #; */
          /* border: 1px solid #; */
      ...
      .CircularProgressbar-text {
        color: ${(props) => (props.needDarkMode ? "#e5e5e5" : "#333")};
        /* border: 1px solid ${(props) =>
          props.needDarkMode ? "#000000" : "#f1f6f1"};
    ...
    .top-label {
      ...
      .selected {
        ...
        background-color: ${(props) =>
          props.needDarkMode ? "#e5e5e5" : "#f0f0f0"};
      }
    }
  }
